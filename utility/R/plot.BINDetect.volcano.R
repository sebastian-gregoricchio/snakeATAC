...
#' @param title String indicating the title to use (markdown annotation supported). Default: \code{NULL} (results in "ConditionA vs ConditionB").
#'
...
plot.BINDetect.volcano =
  function(results,
    ...
    if (is.null(title)) {
      ...
      title = paste0("**",condition.A, "** *vs* **", condition.B, "**")
    }
    ...
    # make plot simmetric
    # get the x-range max
    x.max = max(abs(ggplot_build(plot.BINDetect)$layout$panel_params[[1]]$x.range))

    plot.BINDetect = plot.BINDetect + xlim(c(-1,1) * x.max)
    ...
    if (add.signif.lables == TRUE) {
      plot.BINDetect =
        plot.BINDetect +
        ggrepel::geom_text_repel(data = results %>% dplyr::filter(diff.status != "NS" | motif_id %in% extra.labels),
    ...
    } else if (extra.labels[[1]] != "none") {
      tb.extra = results %>% dplyr::filter(motif_id %in% extra.labels)
      
      if (nrow(tb.extra)) {
        plot.BINDetect =
          plot.BINDetect +
          ggrepel::geom_text_repel(data = tb.extra,
                                   mapping = aes(x = change,
                                                 y = -log10(pvalue),
                                                 label = motif_id,
                                                 color = diff.status),
