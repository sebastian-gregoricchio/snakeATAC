...
if (eval(str(config["perform_HMCan_correction"]))):
    ...
    norm_outputs.append(expand(os.path.join("05_Peaks_MACS2/", "{sample}_mapQ{MAPQ}_woMT_dedup_shifted_FDR{fdr}_peaks.xls"), sample=SAMPLENAMES, MAPQ=str(config["mapQ_cutoff"]), fdr=str(config["FDR_cutoff"]))) # peaks_xls
    norm_outputs.append(expand(os.path.join("05_Peaks_MACS2/", "{sample}_mapQ{MAPQ}_woMT_dedup_shifted_FDR{fdr}_peaks.narrowPeak"), sample=SAMPLENAMES, MAPQ=str(config["mapQ_cutoff"]), fdr=str(config["FDR_cutoff"]))) # narrowPeaks
...
rule A_build_environment:
    output:
        ...
        build_SAM = os.path.dirname("01b_SAM_tempFolder/"),
...
rule E_sam_to_bam:
    ...
    shell:
        ...
        samtools idxstats -@ {params.CPUs} {output.filtBAM_sorted} | cut -f 1 | grep -v MT | xargs samtools view -@ {params.CPUs} -b {output.filtBAM_sorted} > {output.filtBAM_sorted_woMT}
...
rule F_bam_deduplication:
    ...
    output:
        dedup_BAM = temp(os.path.join("03_BAM_dedup/", ''.join(["{SAMPLES}_mapQ", str(config["mapQ_cutoff"]), "_sorted_woMT_dedup.bam"]))),
        dedup_BAM_index = temp(os.path.join("03_BAM_dedup/", ''.join(["{SAMPLES}_mapQ", str(config["mapQ_cutoff"]), "_sorted_woMT_dedup.bai"]))),
...
rule G_bam_shifting:
    input:
        dedup_BAM = ancient(os.path.join("03_BAM_dedup/", ''.join(["{SAMPLES}_mapQ", str(config["mapQ_cutoff"]), "_sorted_woMT_dedup.bam"]))),
        dedup_BAM_index = ancient(os.path.join("03_BAM_dedup/", ''.join(["{SAMPLES}_mapQ", str(config["mapQ_cutoff"]), "_sorted_woMT_dedup.bai"])))
    output:
...
rule J_fragment_size_distribution:
    input:
        BAM = ancient(os.path.join("03_BAM_dedup/", ''.join(["{SAMPLES}_mapQ", str(config["mapQ_cutoff"]), "_sorted_woMT_dedup.bam"]))),
        BAM_index = ancient(os.path.join("03_BAM_dedup/", ''.join(["{SAMPLES}_mapQ", str(config["mapQ_cutoff"]), "_sorted_woMT_dedup.bai"])))
    output:
